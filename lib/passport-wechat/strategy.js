...
WechatStrategy.prototype.authenticate = function (req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    var methodName = userAgent.indexOf('MicroMessenger') > -1 ? 'getAuthorizeURL' : 'getAuthorizeURLForWebsite';

    if (methodName === 'getAuthorizeURL') {
      scope = req.query.silence === 'true' ? 'snsapi_base' : 'snsapi_userinfo';
    } else {
      scope = 'snsapi_login';
    }
